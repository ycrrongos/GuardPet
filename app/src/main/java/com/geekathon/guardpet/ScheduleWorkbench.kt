...
/**
 * 多日日程工作台 UI 逻辑，可挂在 [ScheduleActivity] 或 Compose [AndroidView] 上。
 *
 * 月历控件使用 kizitonwose/Calendar；日程数据、AI 和守护规则由守伴管理。署名见关于页鸣谢。
 */
class ScheduleWorkbench(
    ...
    private val scroll: View
        get() = when {
            root.id == R.id.scheduleScroll -> root
            else -> root.findViewById(R.id.scheduleScroll) ?: root
        }
    ...
    /**
     * 日历权限页返回后继续导入；独立 Activity 与嵌入 Tab 都可调用。
     */
    fun onResume() {
        if (destroyed || !started) return
    ...
    private fun bindViews() {
        calendar = root.findViewById(R.id.scheduleCalendar)
        monthTitle = root.findViewById(R.id.scheduleMonthTitle)
        selectedDateLabel = root.findViewById(R.id.scheduleSelectedDate)
        selectedCountLabel = root.findViewById(R.id.scheduleSelectedCount)
        list = root.findViewById(R.id.scheduleList)
        empty = root.findViewById(R.id.scheduleEmpty)
        aiButton = root.findViewById(R.id.scheduleAiButton)
        guardStatus = root.findViewById(R.id.scheduleAiHint)

        val legend = root.findViewById<LinearLayout>(R.id.scheduleWeekLegend)
    ...
    private fun bindActions() {
        val close = root.findViewById<AppCompatImageButton>(R.id.scheduleCloseButton)
        if (activity is ScheduleActivity) {
            close.setOnClickListener { activity.finish() }
        } else {
            close.visibility = View.GONE
        }
        root.findViewById<AppCompatImageButton>(R.id.schedulePreviousMonth).setOnClickListener {
        ...
        root.findViewById<AppCompatImageButton>(R.id.scheduleNextMonth).setOnClickListener {
        ...
        root.findViewById<AppCompatButton>(R.id.scheduleTodayButton).setOnClickListener {
        ...
        root.findViewById<AppCompatButton>(R.id.scheduleAddButton).setOnClickListener {
            showEditDialog(null)
        }
        aiButton.setOnClickListener { showAiDialog() }
        root.findViewById<AppCompatButton>(R.id.scheduleImportButton).setOnClickListener {
        ...
        root.findViewById<AppCompatButton>(R.id.scheduleHabitButton).setOnClickListener {
            activity.startActivity(android.content.Intent(activity, HabitGuardianActivity::class.java))
        }
    }
    ...
    private fun updateMonthTitle() {
        monthTitle.text = activity.getString(
    ...
    private fun renderSelectedDate() {
        ...
        selectedDateLabel.text = activity.getString(
            R.string.schedule_selected_date_format,
            selectedDate.monthValue,
            selectedDate.dayOfMonth,
            selectedDate.dayOfWeek.getDisplayName(TextStyle.FULL, Locale.getDefault())
        )
        selectedCountLabel.text = activity.getString(R.string.schedule_count_format, schedules.size)
        list.removeAllViews()
    ...
    private fun bindScheduleRow(row: View, schedule: DaySchedule) {
        ...
        row.findViewById<TextView>(R.id.scheduleRowTime).text = activity.getString(
    ...
    private fun buildPolicySummary(schedule: DaySchedule): String {
        ...
        val lock = if (schedule.isPolicyLocked()) {
            " · ${activity.getString(R.string.schedule_locked_short)}"
        } else {
            ""
        }
        return activity.getString(R.string.schedule_policy_counts, allow, block) + lock
    }
    ...
    private fun importCalendarRange() {
        ...
        root.findViewById<AppCompatButton>(R.id.scheduleImportButton).isEnabled = false
        io.execute {
            val result = CalendarScheduleImporter.importUpcoming(activity)
            mainHandler.post {
                if (destroyed) return@post
                root.findViewById<AppCompatButton>(R.id.scheduleImportButton).isEnabled = true
    ...
    private fun brandColor(): Int = ContextCompat.getColor(activity, R.color.brand_primary)
    private fun textColor(): Int = ContextCompat.getColor(activity, R.color.text_primary)
